...
eps=${12}
    ...
    python attack.py --pgd --subs-chk-name ${subs_chk} --subs-net ${subs_net} --target-net ${target_net} --target-chk-name ${target_chk} --centroid-out-name ${cname}  --seed ${seed} --gpu ${gpu} --overwrite --n-variants ${nvars} --gauss-std ${gaussstd} --pgd-steps ${pgd_steps} --pgd-lr ${pgd_lr} --eps ${eps}
